...
![CI](https://github.com/kodakku22/valorant-vc-translator/actions/workflows/ci.yml/badge.svg)
...
vc_translator/    本体
  webapp.py       pywebview アプリのエントリ
  bridge.py       JS↔Python ブリッジ(全 API・セットアップ・設定書き戻し)
  webui/          フロントエンド(HTML/CSS/JS + 同梱フォント)
  main.py         エントリポイント(引数なし→GUI、--console 等はCLIモード)
  paths.py        exe化時/開発時のパス解決
  audio.py        音声取り込み(WASAPI / リサンプリング / 自動再接続 / WAV共通ローダ)
  vad.py          Silero VAD による発話区間検出(強制区切り付き)
  stt.py          faster-whisper 文字起こし(文脈プロンプト / 信頼度 / 正規化)
  translate.py    Ollama 翻訳(文脈解決 / 用語強制 / 共通チャットクライアント)
  suggest.py      返答提案・日→英変換・フィードバック・フレーズ解説
  history.py      SQLite 履歴 + 音声クリップ + SRS + 高精度リスコア
  playback.py     クリップ再生(速度 / 単語サブレンジ)
  overlay.py      字幕オーバーレイ(クリック透過 / 二段表示)/ コンソール表示
  pipeline.py     スレッドパイプライン(遅延ログ / Ollama 自動復帰)
  crashlog.py     未捕捉例外のログ保存
tests/            pytest スイート(CI で自動実行)
```
...
pytest                    :: ロジックテスト(CI と同じ)
ruff check vc_translator  :: lint
```

CI(GitHub Actions)は push ごとに byte-compile / pytest / lint / Web UI 構文チェックを実行。

## ライセンス・免責

- 本体は **Apache License 2.0**。詳細は [LICENSE](LICENSE)。
- 同梱する第三者ライブラリ・フォントの許諾は [THIRD-PARTY-NOTICES.txt](THIRD-PARTY-NOTICES.txt)。
- **VALORANT は Riot Games, Inc. の商標です。本ツールは非公式のファンメイドであり、
  Riot Games とは一切関係ありません**(提携・承認・スポンサー関係なし)。
